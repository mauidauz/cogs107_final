      ...
      # Transform into SDT format (hits, misses, false alarms, correct rejections)
        ...
        for pnum in grouped['pnum'].unique():
            ...
            for condition in p_data['condition'].unique():
                ...
                signal_trials = c_data[c_data['signal'] == 1]  # signal PRESENT
                noise_trials = c_data[c_data['signal'] == 0]  # signal ABSENT
                ...
                if not signal_trials.empty and not noise_trials.empty:
                    ...
                    print(f" Skipping participant {pnum}, condition {condition} — "
                          f"signal_trials: {len(signal_trials)}, noise_trials: {len(noise_trials)}")
...
def apply_hierarchical_sdt_model(data):
    ...
    if data.empty:
        raise ValueError(" Cannot apply SDT model: input data is empty. Check data preprocessing.")
...
def draw_delta_plots(data, pnum):
    ...
    marker_style = {
     ...
     # Create delta plots condition pairs
...
if __name__ == "__main__":
    ...
    # 1. Load and preprocess data for SDT
    sdt_data = read_data(data_path, prepare_for='sdt', display=True)

    # 2. Build and sample from hierarchical SDT model
    model = apply_hierarchical_sdt_model(sdt_data)
    with model:
        trace = pm.sample(1000, tune=1000, target_accept=0.9)

    # 3. Plot and save trace plot
    az.plot_trace(trace)
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / 'trace_plot.png')
    plt.close()

    # 4. Plot and save posterior distributions for key parameters
    az.plot_posterior(trace, var_names=['mean_d_prime', 'mean_criterion'], hdi_prob=0.95)
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / 'posterior_distributions.png')
    plt.close()

    # 5. Load RT data prepared for delta plots
    rt_data = read_data(data_path, prepare_for='delta plots', display=True)
    unique_participants = sorted(rt_data['pnum'].unique())

    # 6. Generate and save delta plots for participants 1 through 10
    for pnum in unique_participants[:10]:
